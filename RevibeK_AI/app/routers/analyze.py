...
from app.services import db_service, download_service, embedding_service, essentia_service
...
async def analyze_music(request: AnalyzeRequest):
    ...
    동작 흐름:
        1. embedding_songs(AUDIO_9D)에 이미 벡터가 있으면 SKIPPED 반환 (경고 로그 출력)
    ...
    # ── 1. embedding_songs에 이미 AUDIO_9D 벡터가 있으면 건너뜀 ────────────────
    # 저장된 벡터를 응답에 함께 돌려줘서, 호출자가 Qdrant 색인 여부와 무관하게
    # 검색에 바로 활용할 수 있게 한다.
    existing_embedding = db_service.get_audio_embedding(request.youtube_video_id)
    if existing_embedding is not None:
        logger.warning(
            "[SKIP] embedding_songs에 이미 AUDIO_9D 벡터가 존재합니다: %s",
        ...
        return AnalyzeResponse(
            ...
            title=request.title,
            status="SKIPPED",
            message=f"이미 분석 결과가 존재합니다 (embedding_songs: {request.youtube_video_id}). 중복 분석을 건너뜁니다.",
