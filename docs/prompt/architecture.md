...
- 플레이리스트에 곡을 여러 개 한 번에 담는 `POST /api/playlists/{playlistId}/items/batch`(다건 추가) API가 추가되었다(7.7 참고). `songIds` 배열을 받아 곡별로 추가하며, 이미 담긴 곡/존재하지 않는 곡은 `skipped`로 분리해 응답한다.
- `POST /api/radio`(라디오 생성) 시 추천곡을 DB 추천 + Qdrant 유사곡 검색으로 확장(최대 8곡)하고, 그 결과로 플레이리스트를 자동 생성해 응답에 `playlistId`로 내려준다(6.1 참고). 즉 "라디오 추천 결과를 플레이리스트에 한 번에 담는" 플로우가 백엔드에서 자동 처리된다.
...
**Service**: `PlaylistService`
**DTO**: `PlaylistDto`, `PlaylistItemDto`, `PlaylistItemBatchRequestDto`, `PlaylistItemBatchResponseDto`
...
- `POST /api/playlists/{playlistId}/items/batch` — 플레이리스트에 곡 여러 개 한 번에 추가
...
**관련 Service**: `RadioService.createRadio()`, `AiDjMentService`, `TtsService`, `PreferenceService`, `QdrantService`, `PlaylistService`
...
- `recommendedSongs`는 DB 추천 결과(최대 5곡)를 1순위 곡 기준 Qdrant 유사곡 검색으로 최대 8곡까지 확장한 결과다. `qdrant.enabled=false`(기본값)이면 확장 없이 DB 추천(최대 5곡) 그대로 반환된다.
- `playlistId`는 위 `recommendedSongs`로 자동 생성된 플레이리스트의 ID다(`"{mood} {era} 라디오 - yyyy.MM.dd"` 형식의 이름, `isPublic: false`). `recommendedSongs`가 비어있으면(`DB_EMPTY`) `playlistId`는 `null`이다. FE에서는 결과 화면에 "이 라디오 플레이리스트 보기" 버튼으로 `GET /api/playlists/{playlistId}`(또는 `/playlists/{playlistId}` 라우트)와 연결하면 된다.
...
### 7.7 POST /api/playlists/{playlistId}/items/batch (곡 여러 개 한 번에 추가)
**인증 필요**: 예
**Request Body** (`PlaylistItemBatchRequestDto`, `songIds` 1개 이상 필수):
```json
{
  "songIds": [
...
```
**Response Body** (`PlaylistItemBatchResponseDto`):
```json
{
  "added": [
    { "id": "ps05-...", "playlistId": "pl01-...", "songId": "s011-...", "title": "캔디 (AI 리마스터)", "artist": "H.O.T", "genre": "댄스", "era": "2세대", "youtubeUrl": "https://www.youtube.com/watch?v=dummy011", "youtubeId": "dummy011", "orderNum": 4, "addedAt": "2026-06-14T10:00:00" }
  ],
  "skipped": [
...
```
**관련 DTO**: `PlaylistItemBatchRequestDto`, `PlaylistItemBatchResponseDto`
**관련 Service**: `PlaylistService.addItems()` (내부적으로 `addItem()`을 곡별로 호출)
**프론트엔드 화면**: Qdrant 유사곡 결과(`GET /api/qdrant/similar/{songId}`)나 라디오 추천 결과를 "전체 담기"로 한 번에 플레이리스트에 추가하는 버튼/모달
**주의사항**:
- `songIds`에 이미 담긴 곡이나 존재하지 않는 곡이 섞여 있어도 전체 요청이 실패하지 않고, 해당 곡만 `skipped`로 분리되며 나머지는 `added`로 정상 처리된다.
- `added` 항목 반영 후에는 플레이리스트 상세를 다시 조회(`GET /api/playlists/{playlistId}`)하거나 응답의 `added` 배열을 그대로 목록에 append하면 된다.
...
export const playlistApi = {
  ...
  addItemsBatch: (playlistId, songIds) => api.post(`/api/playlists/${playlistId}/items/batch`, { songIds }),
  // response: { added: PlaylistItemDto[], skipped: { songId, reason }[] }
...
### 14.10 라디오 생성 시 자동 생성된 플레이리스트 활용
`POST /api/radio` 응답에는 `recommendedSongs`로 자동 생성된 플레이리스트의 `playlistId`가 포함되어 있으므로, 프론트엔드에서 별도로 플레이리스트를 생성하고 곡을 하나씩 추가하는 과정이 필요 없다.

```javascript
// radio 생성 응답에서 playlistId를 바로 사용
const radioResult = await radioApi.createRadio(payload)

if (radioResult.playlistId) {
  // 결과 화면에서 "이 라디오 플레이리스트 보기" 버튼 등으로 바로 이동
  const { data: playlist } = await playlistApi.getPlaylist(radioResult.playlistId)
} else {
  // recommendationSource가 DB_EMPTY인 경우 playlistId가 null
}
```

기존 플레이리스트에 곡(예: Qdrant 유사곡 검색 결과)을 여러 개 한 번에 추가하고 싶을 때는 7.7의 배치 추가 API를 사용한다.

```javascript
// 기존 플레이리스트에 곡 여러 개 한 번에 추가
const songIds = similarSongs.map((song) => song.songId)
const { data } = await playlistApi.addItemsBatch(playlist.id, songIds)
// data.added: 추가된 곡 목록, data.skipped: 중복/존재하지 않아 건너뛴 곡 목록
```
...
### 19.4 플레이리스트/좋아요 관련
- `POST /api/radio`는 추천곡으로 플레이리스트를 자동 생성해 `playlistId`를 응답에 포함한다(14.10 참고). 다만 `recommendedSongs`가 비어있으면(`DB_EMPTY`) `playlistId`는 `null`이므로 프론트에서 분기 처리가 필요하다.
- 기존 플레이리스트에 곡을 여러 개 한 번에 추가할 때는 `POST /api/playlists/{playlistId}/items/batch`(7.7 참고)를 사용한다. 응답이 `added`/`skipped`로 나뉘므로, UI에서는 부분 실패(일부 곡만 추가됨) 처리를 고려해야 한다.
